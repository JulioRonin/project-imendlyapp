...
const inputCls =
  'w-full h-14 px-5 rounded-[1.25rem] bg-[#FAFBF8] text-[#151714] text-[14px] font-semibold placeholder:text-[#A8ADA6] placeholder:font-medium outline-none focus:ring-2 focus:ring-primary/30 transition-shadow disabled:text-[#70756E]';
      ...
      <header className="v2-rise sticky top-0 z-50 bg-[#F3F4F1]/85 backdrop-blur-xl">
        <div className="max-w-3xl mx-auto px-6 py-5 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <h1 className="text-[22px] font-semibold tracking-tight text-[#151714] leading-tight">Perfil</h1>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-3xl mx-auto px-6 mt-2 space-y-8">
        ...
        <section className="v2-rise v2-d2 flex flex-col items-center text-center">
          <Avatar
            ...
            size="xl"
            className="w-28 h-28 text-3xl ring-4 ring-[#E9F7EF] v2-shadow-lift mb-5"
          />
          <h2 className="text-[22px] font-bold tracking-tight text-[#151714] mb-1">{personalInfo.name}</h2>
          <p className="text-[14px] font-medium text-[#70756E] mb-4">{personalInfo.email}</p>
          <span className="inline-flex items-center gap-1.5 h-8 px-3.5 rounded-full bg-[#E9F7EF] text-[#2A9460] text-[11px] font-bold">
        ...
        <Reveal>
          <section>
            <div className="flex items-center justify-between mb-4 px-1">
              <h3 className="text-xl font-semibold tracking-tight text-[#151714]">Información básica</h3>
              {!isEditingInfo && (
                <button
            ...
            <div className="bg-white rounded-[2.25rem] v2-shadow-soft overflow-hidden">
              {isEditingInfo ? (
                <div className="p-6 space-y-4">
                  <div>
                    <label className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#A8ADA6] mb-2 ml-1 block">
                      Nombre completo
                    </label>
                    <input
                ...
                <>
                  <div className="flex items-center gap-4 px-6 py-5">
                    <span className="w-11 h-11 shrink-0 rounded-[1.05rem] bg-[#E9F7EF] text-primary flex items-center justify-center">
                      <User size={19} />
                    </span>
                    <div className="flex-1 min-w-0">
                      <p className="text-[11px] font-medium text-[#A8ADA6]">Nombre completo</p>
                      <p className="text-[14.5px] font-semibold text-[#151714] truncate">{personalInfo.name || '—'}</p>
                    </div>
                  </div>
                  <div className="border-t border-black/[0.06] flex items-center gap-4 px-6 py-5">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[11px] font-medium text-[#A8ADA6]">Teléfono celular</p>
                      <p className="text-[14.5px] font-semibold text-[#151714] truncate tabular-nums">{personalInfo.phone || 'Sin registrar'}</p>
                    </div>
                  </div>
                  <div className="border-t border-black/[0.06] flex items-center gap-4 px-6 py-5">
                    <span className="w-11 h-11 shrink-0 rounded-[1.05rem] bg-[#E9F7EF] text-primary flex items-center justify-center">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[11px] font-medium text-[#A8ADA6]">Correo electrónico</p>
                      <p className="text-[14.5px] font-semibold text-[#151714] truncate">{personalInfo.email || '—'}</p>
                    </div>
                  </div>
                </>
              )}
        ...
        <Reveal delay={60}>
          <section>
            <div className="flex items-center justify-between mb-4 px-1">
              <h3 className="text-xl font-semibold tracking-tight text-[#151714]">Métodos de pago</h3>
              <button
                onClick={() => setIsAddingPayment(true)}
                aria-label="Agregar método de pago"
                className="w-10 h-10 rounded-full bg-[#E9F7EF] text-primary flex items-center justify-center v2-press hover:bg-primary hover:text-white transition-colors"
              >
            ...
            <div className="bg-white rounded-[2.25rem] v2-shadow-soft overflow-hidden">
              {paymentMethods.map((method: any, i: number) => (
                <div key={method.id} className={`flex items-center gap-4 px-6 py-5 group ${i > 0 ? 'border-t border-black/[0.06]' : ''}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[14.5px] font-semibold text-[#151714]">
                      {method.type} terminada en {method.last4}
                    </p>
                    <p className="text-[12px] font-medium text-[#70756E]">
                      Expira {method.exp} {method.isDefault && <span className="text-primary font-semibold">· Principal</span>}
                    </p>
                  </div>
                  <button className="text-[#A8ADA6] hover:text-red-500 transition-colors p-2 v2-press" aria-label="Eliminar método">
                    <Trash2 size={17} />
                  </button>
                </div>
              ))}
              ...
              {isAddingPayment ? (
                <div className={`p-6 ${paymentMethods.length > 0 ? 'border-t border-black/[0.06]' : ''}`}>
                  <h4 className="text-[15px] font-semibold tracking-tight text-[#151714] mb-5">Agregar nueva tarjeta / cuenta</h4>
                ...
                paymentMethods.length === 0 && (
                  <button
                    onClick={() => setIsAddingPayment(true)}
                    className="w-full flex items-center gap-4 px-6 py-5 text-left v2-press hover:bg-[#FAFBF8] transition-colors"
                  >
                    <span className="w-11 h-11 shrink-0 rounded-[1.05rem] bg-[#E9F7EF] text-primary flex items-center justify-center">
                      <Plus size={19} strokeWidth={2.5} />
                    </span>
                    <span className="flex-1">
                      <span className="block text-[14.5px] font-semibold text-[#151714]">Agregar método de pago</span>
                      <span className="block text-[12px] font-medium text-[#70756E]">Tarjeta o cuenta bancaria</span>
                    </span>
                    <ChevronRight size={17} className="text-[#A8ADA6]" />
                  </button>
                )
              )}
            </div>
        ...
        <Reveal delay={120}>
          <section>
            <div className="flex items-center justify-between mb-4 px-1">
              <h3 className="text-xl font-semibold tracking-tight text-[#151714]">Direcciones guardadas</h3>
              <button
            ...
            <div className="bg-white rounded-[2.25rem] v2-shadow-soft overflow-hidden">
              {addresses.map((address: any, i: number) => (
                <div key={address.id} className={`flex items-center gap-4 px-6 py-5 group ${i > 0 ? 'border-t border-black/[0.06]' : ''}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[14.5px] font-semibold text-[#151714] flex items-center gap-2.5">
                      {address.title}
                      {address.is_default && (
                        <span className="inline-flex items-center h-6 px-2.5 rounded-full bg-[#E9F7EF] text-[#2A9460] text-[10px] font-bold">
                          Principal
                        </span>
                      )}
                    </p>
                    <p className="text-[13px] font-medium text-[#70756E] truncate">{address.street}</p>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <button
                      ...
                      className="text-[#A8ADA6] hover:text-red-500 transition-colors p-2 v2-press"
                    >
                      <Trash2 size={17} />
                    </button>
                    <ChevronRight size={17} className="text-[#A8ADA6]" />
              ...
              {isAddingAddress ? (
                <div className={`p-6 ${addresses.length > 0 ? 'border-t border-black/[0.06]' : ''}`}>
                  <h4 className="text-[15px] font-semibold tracking-tight text-[#151714] mb-5">Nueva dirección</h4>
                ...
                <button
                  onClick={() => setIsAddingAddress(true)}
                  className={`w-full flex items-center gap-4 px-6 py-5 text-left v2-press hover:bg-[#FAFBF8] transition-colors ${addresses.length > 0 ? 'border-t border-black/[0.06]' : ''}`}
                >
                  <span className="w-11 h-11 shrink-0 rounded-[1.05rem] bg-[#E9F7EF] text-primary flex items-center justify-center">
                    <Plus size={19} strokeWidth={2.5} />
                  </span>
                  <span className="flex-1">
                    <span className="block text-[14.5px] font-semibold text-[#151714]">Agregar dirección</span>
                    <span className="block text-[12px] font-medium text-[#70756E]">Casa, oficina u otro lugar</span>
                  </span>
                  <ChevronRight size={17} className="text-[#A8ADA6]" />
                </button>
              )}
