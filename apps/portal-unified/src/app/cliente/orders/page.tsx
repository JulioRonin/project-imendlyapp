...
const statusPillClass = (status: string) => {
  if (status === 'COMPLETED' || status === 'PAID') return 'bg-[#FFD0B4] text-[#2A9460]';
      ...
      <header className="v2-rise sticky top-0 z-50 bg-[#FAF8F4]/85 backdrop-blur-xl">
        <div className="max-w-5xl mx-auto px-6 py-5 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-primary-deep">Tu actividad</p>
            <h1 className="text-[22px] font-semibold tracking-tight text-[#111111] leading-tight">Mis órdenes</h1>
          </div>
          <span className="shrink-0 h-9 px-4 inline-flex items-center rounded-full bg-white v2-shadow-soft text-[12px] font-bold text-[#111111] tabular-nums">
      ...
      <div className="max-w-5xl mx-auto px-6 mt-2">
        ...
        {!isLoading && myOrders.length === 0 && (
          <div className="v2-rise v2-d1 bg-white rounded-[2.25rem] v2-shadow-soft px-8 py-16 flex flex-col items-center text-center">
            ...
            <h2 className="text-[19px] font-semibold tracking-tight text-[#111111] mb-2">
              Aún no tienes órdenes
            </h2>
            <p className="text-[14px] font-medium text-[#6F6B66] max-w-xs mb-8">
              ...
              <article className="bg-white rounded-[1.75rem] p-6 v2-shadow-soft v2-press v2-float group">
                ...
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div className="min-w-0">
                    <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#A39E97] mb-1 tabular-nums">
                      ORD-{p.id}
                    </p>
                    <h3 className="text-[17px] font-semibold tracking-tight text-[#111111] leading-snug">
                      {p.serviceName}
                    </h3>
                  </div>
                  <p className="shrink-0 text-[24px] font-bold tracking-tight text-[#111111] tabular-nums">
                    ${p.price.toFixed(2)}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-4 mb-5 min-w-0">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="text-[13px] font-semibold text-[#6F6B66] truncate">{p.providerName}</span>
                  </div>
                  <div className="flex items-center gap-1.5 text-[#A39E97] shrink-0">
                    <Calendar size={13} />
                ...
                {p.status === 'COUNTER_OFFER' && p.counterOffer && (
                  <div className="mb-5 p-5 rounded-[1.25rem] bg-amber-50 flex flex-col sm:flex-row sm:items-center gap-4">
                    <div className="flex-1">
                      ...
                      <p className="text-[14px] font-semibold text-[#111111] tabular-nums">
                    ...
                    <button
                      ...
                      className="shrink-0 h-11 px-5 rounded-full bg-[#111111] text-white text-[12px] font-bold v2-press"
                    >
                      Aceptar horario
                    </button>
                  </div>
                ...
                <div className="mt-5 flex items-center gap-3">
                  {p.status === 'ACCEPTED' && (
                    <button
                      ...
                      className="flex-1 sm:flex-none h-14 px-8 rounded-full bg-[#111111] text-white text-[13px] font-bold v2-press v2-shadow-lift"
                    >
                      Proceder al pago
                    </button>
                  )}
                  <span className="ml-auto flex items-center gap-2">
                    <span className="w-11 h-11 rounded-full bg-[#FFFFFF] text-[#A39E97] flex items-center justify-center group-hover:bg-[#FFD0B4] group-hover:text-primary-deep transition-colors">
                      <MessageCircle size={18} />
                    </span>
                    <ChevronRight size={19} className="text-[#A39E97] group-hover:text-primary-deep group-hover:translate-x-0.5 transition-all" />
                  </span>
                </div>
              </article>
            </div>
