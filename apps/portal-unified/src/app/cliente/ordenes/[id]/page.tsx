...
const statusPill = (status: string) => {
  if (status === 'completed' || status === 'paid') return { label: 'Completado', cls: 'bg-[#E7F2E9] text-[#2A9460]' };
  ...
  if (status === 'scheduled') return { label: 'Agendado', cls: 'bg-[#E7F2E9] text-[#2A9460]' };
  if (status === 'in_progress') return { label: 'En progreso', cls: 'bg-[#E7F2E9] text-[#2A9460]' };
  return { label: status, cls: 'bg-[#E7F2E9] text-[#2A9460]' };
};
...
function OrderDetailsPage() {
  ...
  if (!order) return (
    <div className="min-h-screen bg-[#F4F1EA] flex flex-col items-center justify-center px-8 text-center">
      ...
      <h2 className="text-[19px] font-semibold tracking-tight text-[#1B1A17] mb-2">Orden no encontrada</h2>
      <p className="text-[14px] font-medium text-[#7A7468] mb-8">El enlace es inválido o la orden ya no existe.</p>
      <Link
        href="/cliente"
  ...
  return (
    <main className="min-h-screen bg-[#F4F1EA] pb-36">
      ...
      <header className="v2-rise sticky top-0 z-50 bg-[#F4F1EA]/85 backdrop-blur-xl">
        <div className="max-w-5xl mx-auto px-6 py-5 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <h1 className="text-[22px] font-semibold tracking-tight text-[#1B1A17] leading-tight truncate">
      ...
      <div className="max-w-5xl mx-auto px-6 mt-2 space-y-6">
        ...
        <section className="v2-rise v2-d1 relative overflow-hidden rounded-[2.25rem] bg-[#1B1A17] text-white p-8 md:p-10 v2-shadow-float">
        ...
        <section className="v2-rise v2-d2 bg-white rounded-[2.25rem] p-7 md:p-9 v2-shadow-soft">
          <h2 className="text-xl font-semibold tracking-tight text-[#1B1A17] mb-8">Etapas del servicio</h2>
            ...
            <div className="space-y-3">
              {stages.map((stage) => (
                ...
                >
                  ...
                  {/* Tarjeta de etapa */}
                  <div className={`flex-1 rounded-[1.25rem] p-4 transition-colors ${stage.active && !stage.done ? 'bg-[#E7F2E9]' : stage.done ? 'bg-[#FBF9F4]' : ''}`}>
                    <div className="flex items-center gap-2.5 mb-1">
                      <h4 className={`text-[14.5px] font-semibold tracking-tight ${stage.active || stage.done ? 'text-[#1B1A17]' : 'text-[#ACA598]'}`}>
                        {stage.label}
                      </h4>
                    ...
                    <p className={`text-[13px] font-medium leading-relaxed max-w-md ${stage.active || stage.done ? 'text-[#7A7468]' : 'text-[#ACA598]'}`}>
                      {stage.description}
                    </p>
                  </div>
                </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
            ...
            <section className="bg-white rounded-[2.25rem] p-7 md:p-8 v2-shadow-soft space-y-7 h-full">
              <div className="flex items-center gap-5">
                <Avatar
                  ...
                  className="w-20 h-20 text-xl ring-4 ring-[#E7F2E9]"
                />
                <div className="flex-1 min-w-0">
                  <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-primary mb-1">Tu profesional</p>
                  <div className="flex items-center gap-2.5">
                    <h3 className="text-[19px] font-semibold tracking-tight text-[#1B1A17] truncate">
                      {providerUser.full_name}
                    </h3>
                    {provider.is_top && (
                      <span className="w-6 h-6 shrink-0 bg-[#1B1A17] rounded-full flex items-center justify-center text-[10px] text-white font-bold italic">
                        M
                      </span>
                    )}
                  </div>
                  {provider.is_verified && (
                    <span className="mt-2 inline-flex items-center gap-1.5 h-7 px-3 rounded-full bg-[#E7F2E9] text-[#2A9460] text-[11px] font-bold">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <button className="h-14 rounded-full bg-white border border-black/[0.06] text-[#1B1A17] text-[13px] font-semibold flex items-center justify-center gap-2.5 v2-press hover:bg-[#FBF9F4] transition-colors">
          ...
          <Reveal delay={90}>
            <section className="bg-white rounded-[2.25rem] p-7 md:p-8 v2-shadow-soft h-full">
              <h2 className="text-xl font-semibold tracking-tight text-[#1B1A17] mb-7">Detalles del servicio</h2>

              <div className="space-y-5">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#ACA598] mb-1.5">
                    Servicio contratado
                  </p>
                  <p className="text-[15px] font-semibold text-[#1B1A17]">{serviceName}</p>
                </div>
                ...
                <div className="pt-5 border-t border-black/[0.06]">
                  ...
                  <p className="text-[15px] font-semibold text-[#1B1A17] flex items-center gap-2">
                ...
                <div className="pt-5 border-t border-black/[0.06] flex items-end justify-between gap-4">
                  <div>
                    ...
                    <p className="text-[28px] font-bold tracking-tight text-[#1B1A17] tabular-nums">{total}</p>
                  </div>
                  <span className={`inline-flex items-center h-8 px-3.5 rounded-full text-[11px] font-bold ${order.status === 'cancelled' ? 'bg-red-50 text-red-600' : 'bg-[#E7F2E9] text-[#2A9460]'}`}>
                    {order.status === 'cancelled' ? 'Cancelado' : 'Pagado'}
                  </span>
                </div>
              </div>
      ...
      {isCancelModalOpen && (
        <div className="fixed inset-0 bg-[#1B1A17]/60 backdrop-blur-md z-[100] flex items-center justify-center p-6">
          <div className="v2-scale max-w-md w-full bg-white rounded-[2.25rem] p-8 v2-shadow-float">
            <h3 className="text-[22px] font-semibold tracking-tight text-[#1B1A17] mb-2">Cancelar servicio</h3>
            <p className="text-[13.5px] font-medium text-[#7A7468] leading-relaxed mb-7">
              ...
              {CANCEL_REASONS.map((reason) => (
                <button
                  ...
                  className={`w-full px-4 py-3.5 rounded-[1.25rem] text-left flex items-center gap-3.5 v2-press transition-colors ${
                    selectedReason === reason ? 'bg-[#E7F2E9]' : 'bg-[#FBF9F4] hover:bg-[#F4F1EA]'
                  }`}
                >
                  ...
                  <span className={`text-[13.5px] font-semibold ${selectedReason === reason ? 'text-[#1B1A17]' : 'text-[#7A7468]'}`}>
                    {reason}
                  </span>
                </button>
              ))}
