...
const statusPill = (status: string) => {
  if (status === 'completed' || status === 'paid') return { label: 'Completado', cls: 'bg-[#FFD0B4] text-[#2A9460]' };
  ...
  if (status === 'scheduled') return { label: 'Agendado', cls: 'bg-[#FFD0B4] text-[#2A9460]' };
  if (status === 'in_progress') return { label: 'En progreso', cls: 'bg-[#FFD0B4] text-[#2A9460]' };
  return { label: status, cls: 'bg-[#FFD0B4] text-[#2A9460]' };
};
...
function OrderDetailsPage() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#FAF8F4] flex items-center justify-center">
      <Loader2 size={40} className="text-primary-deep animate-spin" />
    </div>
  );

  if (!order) return (
    <div className="min-h-screen bg-[#FAF8F4] flex flex-col items-center justify-center px-8 text-center">
      <span className="w-20 h-20 rounded-[1.5rem] bg-[#FFD0B4] text-primary-deep flex items-center justify-center mb-6">
        <XCircle size={32} strokeWidth={1.8} />
      </span>
      <h2 className="text-[19px] font-semibold tracking-tight text-[#111111] mb-2">Orden no encontrada</h2>
      <p className="text-[14px] font-medium text-[#6F6B66] mb-8">El enlace es inválido o la orden ya no existe.</p>
      <Link
        href="/cliente"
        className="h-14 px-8 inline-flex items-center rounded-full bg-primary text-ink text-[13px] font-bold shadow-lg shadow-primary/25 v2-press hover:bg-primary-dark transition-colors"
      >
        Volver al inicio
      </Link>
    </div>
  ...
  return (
    <main className="min-h-screen bg-[#FAF8F4] pb-36">
      ...
      <header className="v2-rise sticky top-0 z-50 bg-[#FAF8F4]/85 backdrop-blur-xl">
        <div className="max-w-5xl mx-auto px-6 py-5 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-primary-deep tabular-nums">
              Orden · {order.display_id}
            </p>
            <h1 className="text-[22px] font-semibold tracking-tight text-[#111111] leading-tight truncate">
      ...
      <div className="max-w-5xl mx-auto px-6 mt-2 space-y-6">
        ...
        <section className="v2-rise v2-d1 relative overflow-hidden rounded-[2.25rem] bg-[#111111] text-white p-8 md:p-10 v2-shadow-float">
          <div className="absolute -top-20 -right-20 w-72 h-72 rounded-full bg-primary/25 blur-3xl pointer-events-none" />
          <div className="relative flex items-end justify-between gap-6">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-primary-deep mb-3">
                Monto del servicio
              </p>
            ...
            <span className={`shrink-0 inline-flex items-center h-9 px-4 rounded-full text-[11px] font-bold ${order.status === 'cancelled' ? 'bg-red-500/15 text-red-300' : 'bg-primary/15 text-primary-deep'}`}>
        ...
        <section className="v2-rise v2-d2 bg-white rounded-[2.25rem] p-7 md:p-9 v2-shadow-soft">
          <h2 className="text-xl font-semibold tracking-tight text-[#111111] mb-8">Etapas del servicio</h2>
            ...
            <div className="space-y-3">
              {stages.map((stage) => (
                ...
                >
                  ...
                  {/* Tarjeta de etapa */}
                  <div className={`flex-1 rounded-[1.25rem] p-4 transition-colors ${stage.active && !stage.done ? 'bg-[#FFD0B4]' : stage.done ? 'bg-[#FFFFFF]' : ''}`}>
                    <div className="flex items-center gap-2.5 mb-1">
                      <h4 className={`text-[14.5px] font-semibold tracking-tight ${stage.active || stage.done ? 'text-[#111111]' : 'text-[#A39E97]'}`}>
                        {stage.label}
                      </h4>
                      {stage.active && !stage.done && (
                        <span className="inline-flex items-center h-6 px-2.5 rounded-full bg-primary text-ink text-[10px] font-bold">
                          Actual
                        </span>
                      )}
                    </div>
                    <p className={`text-[13px] font-medium leading-relaxed max-w-md ${stage.active || stage.done ? 'text-[#6F6B66]' : 'text-[#A39E97]'}`}>
                      {stage.description}
                    </p>
                  </div>
                </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
            ...
            <section className="bg-white rounded-[2.25rem] p-7 md:p-8 v2-shadow-soft space-y-7 h-full">
              <div className="flex items-center gap-5">
                <Avatar
                  ...
                  className="w-20 h-20 text-xl ring-4 ring-[#FFD0B4]"
                />
                <div className="flex-1 min-w-0">
                  <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-primary-deep mb-1">Tu profesional</p>
                  <div className="flex items-center gap-2.5">
                    <h3 className="text-[19px] font-semibold tracking-tight text-[#111111] truncate">
                      {providerUser.full_name}
                    </h3>
                    {provider.is_top && (
                      <span className="w-6 h-6 shrink-0 bg-[#111111] rounded-full flex items-center justify-center text-[10px] text-white font-bold italic">
                        M
                      </span>
                    )}
                  </div>
                  {provider.is_verified && (
                    <span className="mt-2 inline-flex items-center gap-1.5 h-7 px-3 rounded-full bg-[#FFD0B4] text-[#2A9460] text-[11px] font-bold">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button className="h-14 rounded-full bg-primary text-ink text-[13px] font-bold flex items-center justify-center gap-2.5 shadow-lg shadow-primary/25 v2-press hover:bg-primary-dark transition-colors">
                  <MessageCircle size={16} /> Abrir chat
                </button>
                <button className="h-14 rounded-full bg-white border border-black/[0.06] text-[#111111] text-[13px] font-semibold flex items-center justify-center gap-2.5 v2-press hover:bg-[#FFFFFF] transition-colors">
          ...
          <Reveal delay={90}>
            <section className="bg-white rounded-[2.25rem] p-7 md:p-8 v2-shadow-soft h-full">
              <h2 className="text-xl font-semibold tracking-tight text-[#111111] mb-7">Detalles del servicio</h2>

              <div className="space-y-5">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#A39E97] mb-1.5">
                    Servicio contratado
                  </p>
                  <p className="text-[15px] font-semibold text-[#111111]">{serviceName}</p>
                </div>
                ...
                <div className="pt-5 border-t border-black/[0.06]">
                  ...
                  <p className="text-[15px] font-semibold text-[#111111] flex items-center gap-2">
                    <MapPin size={15} className="shrink-0 text-primary-deep" /> {order.address}
                  </p>
                </div>
                ...
                <div className="pt-5 border-t border-black/[0.06] flex items-end justify-between gap-4">
                  <div>
                    ...
                    <p className="text-[28px] font-bold tracking-tight text-[#111111] tabular-nums">{total}</p>
                  </div>
                  <span className={`inline-flex items-center h-8 px-3.5 rounded-full text-[11px] font-bold ${order.status === 'cancelled' ? 'bg-red-50 text-red-600' : 'bg-[#FFD0B4] text-[#2A9460]'}`}>
                    {order.status === 'cancelled' ? 'Cancelado' : 'Pagado'}
                  </span>
                </div>
              </div>
      ...
      {isCancelModalOpen && (
        <div className="fixed inset-0 bg-[#111111]/60 backdrop-blur-md z-[100] flex items-center justify-center p-6">
          <div className="v2-scale max-w-md w-full bg-white rounded-[2.25rem] p-8 v2-shadow-float">
            <h3 className="text-[22px] font-semibold tracking-tight text-[#111111] mb-2">Cancelar servicio</h3>
            <p className="text-[13.5px] font-medium text-[#6F6B66] leading-relaxed mb-7">
              ...
              {CANCEL_REASONS.map((reason) => (
                <button
                  ...
                  className={`w-full px-4 py-3.5 rounded-[1.25rem] text-left flex items-center gap-3.5 v2-press transition-colors ${
                    selectedReason === reason ? 'bg-[#FFD0B4]' : 'bg-[#FFFFFF] hover:bg-[#FAF8F4]'
                  }`}
                >
                  ...
                  <span className={`text-[13.5px] font-semibold ${selectedReason === reason ? 'text-[#111111]' : 'text-[#6F6B66]'}`}>
                    {reason}
                  </span>
                </button>
              ))}
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 h-14 rounded-full bg-white border border-black/[0.06] text-[#111111] text-[13px] font-semibold v2-press hover:bg-[#FFFFFF] transition-colors"
              >
                Volver
              </button>
              <button
